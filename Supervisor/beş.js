  ...
  files.forEach(f => {
    readdir("./src/beş_commands/" + f, (err2, files2) => {
      ...
      files2.forEach(file => {
        ...
        console.log(`🧮 [WEST - COMMANDS] ${beş_prop.name} Yüklendi!`);
...
readdir("./src/beş_events", (err, files) => {
  ...
  files.filter((file) => file.endsWith(".js")).forEach((file) => {
    ...
    console.log(`📚 [WEST _ EVENTS] ${beş_prop.conf.name} Yüklendi!`);
  });
});
...
readdir("./src/beş_trackers", (err, files) => {
  ...
  files.filter((file) => file.endsWith(".js")).forEach((file) => {
    ...
    if (!beş_prop.conf) return;
    client.on(beş_prop.conf.name, beş_prop);
    console.log(`📩 [WEST _ TRACKERS] ${beş_prop.conf.name} Yüklendi!`);
  });
});
...
client.ceza = async function (id, message, type, reason, durations, süre) {
  ...
  let embed = new EmbedBuilder().setColor("#2f3136").setAuthor({ name: message.member.displayName, iconURL: message.author.avatarURL({ dynamic: true, size: 2048 }) }).setFooter({ text: beş_config.footer ? beş_config.footer : `west was here`, iconURL: message.author.avatarURL({ dynamic: true, size: 2048 }) })
  switch (type) {
    case 'BAN':
